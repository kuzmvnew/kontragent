...
def main():
    ...
    fields=("group","company","inn","risk_score","risk_label","overall","coverage_score","mandatory_score","positive_allowed","unresolved","fssp_validation","data_quality_flags")
    with (args.output/"product_recovery_v3_40.csv").open("w",newline="",encoding="utf-8") as f:
        w=csv.DictWriter(f,fieldnames=fields); w.writeheader()
        for x in matrix:
            w.writerow({"group":x["group"],"company":x["company"],"inn":x["inn"],"risk_score":x["risk"]["risk_score"],"risk_label":x["risk"]["label"],"overall":x["risk"]["overall"],"coverage_score":x["risk"]["coverage"]["coverage_score"],"mandatory_score":x["risk"]["coverage"]["mandatory_score"],"positive_allowed":x["summary"]["positive_conclusion_allowed"],"unresolved":" | ".join(x["risk"]["coverage"]["unresolved_capabilities"]),"fssp_validation":x["fssp_validation"],"data_quality_flags":" | ".join(x["data_quality_flags"])})
    ...
    report={"generated_at":now.isoformat(),"target":40,"actual":len(matrix),"buckets":buckets,"coverage":{"min":min(coverage),"max":max(coverage),"average":round(sum(coverage)/len(coverage),1)},"positive_gate_passed":accepted,"persisted":persisted,"reused":reused,"unresolved":unresolved,"runtime":{"fssp_direct":"UNAVAILABLE: no machine transport; browser sessions not completed in this run","efrsb_direct":"UNAVAILABLE: no configured public/machine transport","cbr_zsk":"UNAVAILABLE: interactive public flow not completed","bankinform":"UNAVAILABLE: no company-specific BIK supplied","checko":"ACCESS_PENDING: CHECKO_API_KEY absent"}}
    ...
    for x in matrix:
        ...
        positives="".join(f"<li>{html.escape(v)}</li>" for v in x["summary"]["positive_checks"]) or "<li>Нет завершённых отрицательных результатов для показа</li>"
        ...
        cards.append(f'''<article class="card" data-group="{html.escape(x['group'])}" data-search="{html.escape((x['company']+' '+x['inn']).lower())}"><span>{html.escape(x['group'])}</span><h2>{html.escape(x['company'])}</h2><p>ИНН {x['inn']} · ОГРН {html.escape(str(x['ogrn'] or '—'))}</p><div class="scores"><b>{html.escape(x['summary']['risk_line'])}</b><b>{html.escape(x['summary']['coverage_line'])}</b></div><p>{html.escape(x['summary']['conclusion'])}</p><h3>Главные причины</h3><ul>{reasons}</ul><h3>Подтверждённые проверки</h3><ul>{positives}</ul><h3>Не удалось проверить</h3><ul>{limits}</ul><h3>Что сделать</h3><ul>{recs}</ul><details><summary>Источники и методика</summary><div class="scroll"><table><tr><th>Источник</th><th>Тип</th><th>Дата</th><th>Покрытие</th><th>Баллы</th></tr>{sources}</table></div></details></article>''')
