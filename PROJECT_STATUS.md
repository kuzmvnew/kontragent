...
| 4. Качество данных | ▶️ ACTIVE | Wave 1 до SEO MVP; W1-001 принят на Mac, W1-003 следующий только после отдельной команды |
...
- **W1-002 CBR FinOrg:** по локальному логу от 16.09.2026 — `W1-002 LIVE + CACHE: PASS`, ИНН `7707083893`, ПАО Сбербанк, 10 записей лицензий/прав, 9 active по логике сервиса, повторное чтение `cached=True`. Это точечная интеграция, не массовая загрузка. Отдельная browser acceptance для W1-002 всё ещё не подтверждена и не выводится из результата W1-001.
- **W1-003 ФНС — реестр МСП, получателей поддержки:** NEXT PLANNED, но **НЕ НАЧАТ**. Начинать только отдельной явной командой в новом рабочем чате.
