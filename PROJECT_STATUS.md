...
| 4. Качество данных | ▶️ ACTIVE | Согласованная Wave 1 до SEO MVP; приёмка W1-001 |
...
- На исходном `c2b440b`: **361 passed**. После исправлений W1-001: **384 passed** в изолированной Python 3.13 и повторно в GitHub Actions Python 3.14.
- W1-001 CI run `35094003682`: SUCCESS — PostgreSQL 17, миграции, реальный официальный импорт, повторное чтение, Chromium и rollback. Подробности: `docs/W1_001_ACCEPTANCE.md`. Это не запуск на Mac.
...
- **W1-001 CBR Warning List:** техническая проверка на отдельной PostgreSQL/Chromium CI прошла 16.09.2026: HTTP 200, импорт 27 154 записей, 0 rejected/duplicates, реальный ИНН `0100007874`, CBR ID `29279`, повторное чтение и экран `found`. С ИНН 2 995 записей, без ИНН 24 159. Полный импорт, браузер и покрытие Master Registry на Mac пока НЕ подтверждены; это оставшаяся точка приёмки, не повод начинать W1-003.
- **W1-002 CBR FinOrg:** по переданной Михаилом сводке от 16.09.2026 — `W1-002 LIVE + CACHE: PASS`, ИНН `7707083893`, 10 записей; локально 361 тест, Alembic `e5a7f4c2b9d1`. Это точечная интеграция, не массовая загрузка. Браузер на Mac отдельно не подтверждён.
- **W1-003 МСП — получатели поддержки:** НЕ НАЧАТ. Не начинать без отдельной команды.
- Person, Leads, CRM, Enterprise не начинаем. РНП/ЕИС отложен до официального доступа; DaMIA не подключаем.

Базовый проверенный main перед этой задачей: `c2b440b03b037d8acb295981b998852be11f8aba` (PR #25).
Размер Master Registry выше — историческое подтверждение, не новый подсчёт.
GitHub-изменение не обновляет автоматически файлы и PostgreSQL на Mac.
