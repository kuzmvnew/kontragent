...
def test_summary_html_partial_renders_current_and_empty_states():
    risk = assessment(
        tax_debt_check=check(
            "found", "fns_tax_debt", has_debt=True, total_debt="2000000"
        )
    )
    summary = build_summary(risk, now=NOW).model_dump(mode="json")
    template = Environment(loader=FileSystemLoader("templates")).get_template(
        "partials/summary_engine.html"
    )

    current_html = template.render(
        company={
            "inn": risk.company_inn,
            "risk_assessment": {"assessment_id": risk.assessment_id},
            "summary": summary,
        }
    )
    assert "Краткий итог проверки" in current_html
    assert summary["text_blocks"]["short_conclusion"]["text"] in current_html
    assert f'data-summary-id="{summary["summary_id"]}"' in current_html
    assert "Почему" in current_html
    ...
    assert "Краткий итог ещё не сформирован" in empty_html
    assert "Сформировать краткий итог" in empty_html

    stale_html = template.render(
        company={
            "inn": risk.company_inn,
            "risk_assessment": {"assessment_id": "new-assessment"},
            "summary": summary,
        }
    )
    assert "относится к предыдущей оценке" in stale_html

    no_risk_html = template.render(
        company={"inn": risk.company_inn, "risk_assessment": None, "summary": None}
    )
    assert "Сначала рассчитайте оценку рисков" in no_risk_html
    assert "<form" not in no_risk_html


def test_summary_post_route_success_and_repeat(monkeypatch):
    import main

    calls = []

    def fake_generate(inn):
        calls.append(inn)

    monkeypatch.setattr(main, "generate_company_summary", fake_generate)
    with TestClient(main.app) as client:
        first = client.post("/company/7700000000/summary", follow_redirects=False)
        repeated = client.post("/company/7700000000/summary", follow_redirects=False)

    assert first.status_code == repeated.status_code == 303
    assert first.headers["location"] == repeated.headers["location"] == "/company/7700000000"
    assert calls == ["7700000000", "7700000000"]
    assert any(
        route.path == "/company/{inn}/summary" and "POST" in route.methods
        for route in main.app.routes
    )


def test_summary_post_route_missing_risk_and_invalid_inn_are_not_success(monkeypatch):
    import main

    def missing_risk(_inn):
        raise ValueError("Risk assessment not found")

    monkeypatch.setattr(main, "generate_company_summary", missing_risk)
    with TestClient(main.app) as client:
        missing = client.post("/company/7700000000/summary", follow_redirects=False)
        invalid = client.post("/company/123/summary", follow_redirects=False)

    assert missing.status_code == 404
    assert missing.json() == {"detail": "Risk assessment not found"}
    assert invalid.status_code == 400
    assert "ИНН должен содержать" in invalid.json()["detail"]
