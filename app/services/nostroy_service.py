...
SOURCE_CODE = "nostroy"
DATASET_CODE = "nostroy_sro_members_on_demand"
...
def _result(row, *, cached=True):
    ...
    return build_check_result(
        checked=True,
        applicable=True,
        result="found" if row.is_found else "not_found",
        ...
        active_record_count=sum(r.get("member_status_code") == "1" for r in records),
        cached=cached,
        interpretation_note="Наличие записи подтверждает опубликованное текущее или историческое членство; статус каждой записи проверяется отдельно.",
        coverage_note="Точечная проверка строительного ЕРЧ СРО НОСТРОЙ; не покрывает НОПРИЗ, НРС и все виды СРО России.",
    )


def get_cached_nostroy_check(inn, request_date=None, *, applicable=None):
    inn, request_date = _company_inn(inn), request_date or date.today()
    if not inn or applicable is False:
        return build_check_result(checked=True, applicable=False, result="not_applicable", data_date=None, dataset_code=DATASET_CODE, source=SOURCE_CODE, reason="construction_sro_context_not_applicable", matching_method=None, records=[], record_count=0)
    ...
    session = get_session()
    try:
        row = session.scalar(select(NostroyMemberCheck).where(NostroyMemberCheck.inn == inn, NostroyMemberCheck.request_date == request_date))
        if row is None:
...
def refresh_nostroy_check(inn, request_date=None, provider=None, *, applicable, force_refresh=False):
    inn, request_date = _company_inn(inn), request_date or date.today()
    if not inn:
    ...
    session = get_session()
    try:
        statement = insert(NostroyMemberCheck).values(**values).on_conflict_do_update(index_elements=["inn", "request_date"], set_={key: value for key, value in values.items() if key not in {"inn", "request_date"}}).returning(NostroyMemberCheck)
