...
def build_cbr_warning_source_spec() -> dict:
    return {
        "code": SOURCE_CODE,
        "name": "Банк России",
...
def build_cbr_warning_dataset_spec(source_id: int) -> dict:
    return {
        "source_id": source_id,
        "code": DATASET_CODE,
        "name": (
        ...
        "description": (
            "Официальный JSON предупредительного списка Банка России, "
            "предназначенный для использования в автоматизированных системах."
        ),
    }


def ensure_cbr_warning_list_dataset() -> None:
    session = get_session()

    try:
        source_values = build_cbr_warning_source_spec()
        ...
        dataset_values = build_cbr_warning_dataset_spec(
            source_id=source_id
        )
